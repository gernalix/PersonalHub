...
class FinanceCapsuleTest {
    ...
    @Test fun versionThreeUpgradeKeepsExistingRowsAndCreatesOnlyEmptyFinanceTables() {
        ...
        val owner = PersonalHubDatabase.openTemporary(context, name)
        try {
            assertEquals(4, owner.openHelper.writableDatabase.version)
            assertEquals(43, scalar(owner, "SELECT generation FROM hub_generation"))
            ...
            assertEquals(43, DatabaseVault.validate(context, file))
