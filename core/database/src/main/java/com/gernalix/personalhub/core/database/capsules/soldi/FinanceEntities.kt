...
@Entity(tableName = "finance_products", indices = [Index(value = ["name"], unique = true)])
data class FinanceProduct(@PrimaryKey(autoGenerate = true) val id: Long = 0, val name: String)
...
@Entity(tableName = "finance_transactions", indices = [Index("titleId"), Index("productId"), Index("chainId"), Index("placeId"), Index("occurredAt")], foreignKeys = [
