...
class ResourceHubAdapter(private val context: Context) : HubEntityAdapter {
    ...
    override suspend fun summaries(canonicalIds: Set<String>) =
        if (canonicalIds.isEmpty()) emptyMap()
        else dao.resources(canonicalIds.toList()).filter(::visible).associate { it.id to it.summary() }

    override suspend fun search(query: String, limit: Int) =
        dao.search(query.trim(), limit.coerceIn(1, 100)).filter(::visible).map { it.summary() }

    override suspend fun openTarget(canonicalId: String): HubOpenTarget? {
        val resource = dao.resource(canonicalId)?.takeIf(::visible) ?: return null
    ...
    override suspend fun create(request: HubCreateRequest): HubEntitySummary? {
        ...
        if (
            value.isBlank() ||
            kind !in setOf(
                HubResourceKinds.WEB_URL,
                HubResourceKinds.ANDROID_URI,
                HubResourceKinds.NOTE,
                HubResourceKinds.SAF_DOCUMENT,
            )
        ) return null
        ...
        val resource = HubResource(
            UUID.randomUUID().toString(),
            kind,
            request.suggestedLabel?.trim()?.takeIf(String::isNotBlank),
            value,
            request.extras["persisted_permission"] == "true",
            now,
            now,
        )
        dao.insert(resource)
    ...
    private fun HubResource.summary() = HubEntitySummary(
        ...
        when (kind) {
            HubResourceKinds.NOTE -> context.getString(R.string.hub_resource_note)
            else -> value
        },
        attributes = mapOf(
            "resource_kind" to kind,
            "value" to value,
            "persisted_permission" to persistedPermission.toString(),
        ),
    )
}
