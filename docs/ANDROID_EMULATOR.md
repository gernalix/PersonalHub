...
`start` is idempotent: it reuses a healthy `Pixel_8a`, otherwise starts it from
the CLI with a hidden Qt window, no boot animation, and no snapshot load/save.
Readiness means the discovered target is in ADB state `device` and
`sys.boot_completed=1`. The returned JSON contains the live serial to pass to
every later `adb -s <serial> ...` command. The older
`--target emulator` invocation remains supported.
...
If startup is blocked, inspect
`/tmp/personalhub-pixel_8a-emulator.log`. For an `offline` target, run
`$ANDROID_SDK_ROOT/platform-tools/adb reconnect offline` once and retry
`status`; the helper itself performs one ADB server recovery when listing
devices fails. For a boot timeout, run `stop`, inspect the log, then `start`.
...
failed launch log contains GPU/renderer evidence.
