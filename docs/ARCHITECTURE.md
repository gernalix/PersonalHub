...
PersonalHub v13 added Soldi as a sixth native module without importing a standalone Soldi database. That data-migration restriction remains: no legacy finance database is bundled, opened, attached or imported. Later Soldi iterations superseded the original v13 prohibition on receipt/image handling: the current module has local receipt OCR, generic transaction attachments, and transaction-photo presentation while preserving `personalhub.db` as the only writable database.

The finance capsule owns the canonical finance tables, including products, titles, chains, stores, transactions, tags, accounts and `finance_attachments`. Purchase transactions reference reusable products; general income/expense titles have a separate catalog so salary or transfers do not become products. Chain and tag names are reused after trimming, with exact uniqueness to avoid merging different products or merchants by guesswork. Stores attach finance attributes to the existing `places.uuid`; there is no parallel place catalog. Foreign keys protect referenced products, chains and Places.

Transactions support creation, reading, editing, deletion, notes, tags, currency, date, product selection and external attachments. `finance_attachments` stores references and metadata only; full photos/PDFs are never inserted into SQLite as finance BLOBs. Local images use persisted SAF `content://` URIs and remote images use provider-agnostic URLs. Receipt OCR remains a separate local-first import flow. See [FINANCE_PHOTOS.md](FINANCE_PHOTOS.md) for the photo-storage, thumbnail and future visual-search contract.
