...
object PlaceGeofenceNotifier {
    ...
    fun notifyTransition(context: Context, place: PlaceEntity, transition: PlaceGeofenceTransition, recordedVisit: Boolean) {
        ...
        val pi = PendingIntent.getActivity(
            context,
            notificationId(place.uuid, transition),
            contentIntent,
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentIntent(pi)
            .build()
        HubNotificationPlatform.post(context, notificationId(place.uuid, transition), notification)
    }
    ...
    fun notifyAmbiguous(context: Context, placeCount: Int) {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setAutoCancel(true)
            .build()
        HubNotificationPlatform.post(context, AMBIGUOUS_NOTIFICATION_ID, notification)
    }
    ...
    private fun ensureChannel(context: Context) {
        HubNotificationPlatform.ensureChannel(
            context = context,
            id = CHANNEL_ID,
            name = context.safeString(R.string.geofence_channel_name, "Places geofences"),
            importance = NotificationManager.IMPORTANCE_HIGH,
            description = context.safeString(
                R.string.geofence_channel_description,
                "Notifications for Place ENTER and EXIT automation.",
            ),
        )
    }
