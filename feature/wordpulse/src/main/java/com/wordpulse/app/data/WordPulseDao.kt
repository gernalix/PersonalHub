...
interface WordPulseDao {
    ...
    @Query("SELECT * FROM sessions ORDER BY started_at_utc_ms ASC")
    suspend fun getSessions(): List<WordSession>

    @Query("SELECT * FROM sessions ORDER BY started_at_utc_ms DESC LIMIT 1")
    ...
    @Query("SELECT started_at_utc_ms FROM sessions WHERE id = :sessionId LIMIT 1")
    ...
    @Query(
        ...
        SELECT sessions.id AS id,
               sessions.started_at_utc_ms AS started_at_utc_ms,
               sessions.ended_at_utc_ms AS ended_at_utc_ms,
               COUNT(word_entries.id) AS word_count,
               COUNT(DISTINCT word_entries.normalized_word) AS unique_count
        FROM sessions
        LEFT JOIN word_entries ON word_entries.session_id = sessions.id
        GROUP BY sessions.id
        ORDER BY sessions.started_at_utc_ms DESC
        """,
    )
