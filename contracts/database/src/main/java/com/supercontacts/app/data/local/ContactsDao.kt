...
interface ContactsDao {
    ...
    @Transaction
    @Query("SELECT * FROM contacts WHERE public_id IN (:publicIds)")
    suspend fun getContactsByPublicIds(publicIds: List<String>): List<ContactWithFieldsAndTags>

    @Transaction
    @Query("SELECT DISTINCT contacts.* FROM contacts LEFT JOIN contact_fields ON contact_fields.contact_id=contacts.id WHERE contacts.deleted_at IS NULL AND contacts.archived_at IS NULL AND (:query='' OR contact_fields.value LIKE '%' || :query || '%' COLLATE NOCASE) ORDER BY contacts.updated_at DESC LIMIT :limit")
    suspend fun searchForHub(query: String, limit: Int): List<ContactWithFieldsAndTags>
