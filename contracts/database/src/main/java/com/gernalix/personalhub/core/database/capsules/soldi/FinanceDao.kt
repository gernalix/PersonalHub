...
interface FinanceDao {
    ...
    @Query("SELECT t.*, COALESCE(p.name,n.name,'') AS title,c.name AS chain,l.nickname AS place FROM finance_transactions t LEFT JOIN finance_products p ON p.id=t.productId LEFT JOIN finance_titles n ON n.id=t.titleId LEFT JOIN finance_chains c ON c.id=t.chainId LEFT JOIN places l ON l.uuid=t.placeId WHERE t.uuid IN (:uuids) ORDER BY t.occurredAt DESC,t.id DESC")
